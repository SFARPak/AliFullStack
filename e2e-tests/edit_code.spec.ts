...
test("edit code", async ({ po }) => {
  const editedFilePath = path.join("src", "components", "made-with-alifullstack.tsx");
  ...
  await po.page.getByText("made-with-alifullstack.tsx").click();
  await po.page
    .getByRole("code")
    .locator("div")
  ...
  await po.page
    ...
    .fill("export const MadeWithAliFullStack = ;");
  ...
  expect(editedFile).toContain("export const MadeWithAliFullStack = ;");
});
