...
const TEST_RESPONSES: Record<string, string> = {
  ...
  <alifullstack-add-dependency packages="deno"></alifullstack-add-dependency>
  
  EOM`,
  "add-non-existing-dep": `I'll add that dependency for you.
  
  <alifullstack-add-dependency packages="@angular/does-not-exist"></alifullstack-add-dependency>
  
  EOM`,
  "add-multiple-deps": `I'll add that dependency for you.
  
  <alifullstack-add-dependency packages="react-router-dom react-query"></alifullstack-add-dependency>
  
  EOM`,
  write: `Hello world
  <alifullstack-write path="src/hello.ts" content="Hello world">
...
export function getTestResponse(prompt: string): string | null {
  const match = prompt.match(/\[alifullstack-qa=([^\]]+)\]/);
  if (match) {
